      ...
      <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex gap-2 flex-wrap mb-3">
              {existingImages.map((img, i) => (
                <div key={i} className="relative">
                  <img src={imgUrl(img)} alt={`Image ${i+1}`} className={`w-16 h-16 object-cover rounded-xl border-2 ${primaryIndex === i ? "border-primary-500" : "border-gray-200"}`} />
                  <button type="button" onClick={() => setPrimaryIndex(i)}
                    className={`absolute top-0.5 right-0.5 w-5 h-5 rounded-full text-xs flex items-center justify-center shadow ${primaryIndex === i ? "bg-primary-500 text-gray-900" : "bg-white text-gray-400"}`}>
                    ★
                  </button>
                  ...
                  }} className="absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-error text-white text-xs flex items-center justify-center shadow hover:bg-error transition">
                    ✕
                  </button>
                  {primaryIndex === i && <span className="absolute bottom-0 left-0 right-0 text-center text-xs bg-primary-500 text-gray-900 rounded-b-xl py-0.5">{isArabic ? "رئيسية" : "Main"}</span>}
                </div>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-5 gap-2">
            {[0,1,2,3,4].map(i => (
              <label key={i} className="block cursor-pointer">
                <div className={`aspect-square rounded-xl border-2 border-dashed flex items-center justify-center overflow-hidden transition ${newPreviews[i] ? "border-primary-300" : "border-gray-200 hover:border-primary-300"}`}>
                  {newPreviews[i]
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input type="number" value={form.preparation_time} onChange={e => setForm(f => ({ ...f, preparation_time: e.target.value }))} min="1"
                className="flex-1 border border-gray-200 rounded-xl px-3 py-3 focus:outline-none focus:ring-2 focus:ring-primary-300" />
              <select value={form.time_unit} onChange={e => setForm(f => ({ ...f, time_unit: e.target.value }))}
                className="border border-gray-200 rounded-xl px-2 py-3 focus:outline-none focus:ring-2 focus:ring-primary-300 bg-white text-sm">
              ...
              {variants.map((v, i) => (
                <div key={i} className="flex items-center justify-between bg-primary-50 rounded-lg px-3 py-2">
                  <div>
                    ...
                    {v.id && <span className="text-xs text-success ml-2">✓ saved</span>}
                  </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input type="text" value={newVariant.name} onChange={e => setNewVariant(v => ({ ...v, name: e.target.value }))}
                  placeholder="Name (EN)" className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
                <input type="text" value={newVariant.name_ar} onChange={e => setNewVariant(v => ({ ...v, name_ar: e.target.value }))}
                  placeholder="اسم عربي" className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
              </div>
              <div className="space-y-2">
                {newVariant.options.map((opt, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <input type="text" value={opt.label} onChange={e => updateOption(i, "label", e.target.value)}
                      placeholder="Option" className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-300" />
                    ...
                    {newVariant.options.length > 1 && (
                      <button type="button" onClick={() => removeOption(i)} className="text-gray-400 hover:text-error">✕</button>
                    )}
                  </div>
                ))}
              </div>
              <button type="button" onClick={addOption} className="text-xs text-primary-500 hover:underline">
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
          ...
          <button type="button" onClick={() => setForm(f => ({ ...f, is_available: !f.is_available }))}
            className={`relative w-12 h-6 rounded-full transition-colors ${form.is_available ? "bg-primary-500" : "bg-gray-300"}`}>
